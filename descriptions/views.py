...
import yt_dlp
from text_to_speech_eleven import text_to_speech as eleven_tts
...
@api_view(['POST'])
def generate_audio_eleven(request):
    """
    ...
    try:
        ...
        try:
            ...
            output_path = eleven_tts(temp_file_path)
        ...
        logger.error(f"Error generating audio with ElevenLabs: {str(e)}", exc_info=True)
        return Response({
            'error': f'Error generating audio: {str(e)}',
            'status': 'error'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
